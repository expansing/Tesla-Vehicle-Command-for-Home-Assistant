...
Fleet Telemetry is optional. The receiver accepts the vehicle's mTLS stream and publishes decoded records on a private ZMQ endpoint.

> [!IMPORTANT]
> The receiver and Tesla registration are implemented, but the private bridge that applies decoded telemetry records to Home Assistant entities is still in development. Completing this setup verifies the receiver path, but it does not yet replace the integration's regular Fleet API polling.
...
3. **No inbound home-network port:** Do not enable the local telemetry receiver. Keep the integration's Fleet API polling enabled instead. A future deployment option is a receiver hosted on a public VM, with decoded records returned to Home Assistant over an outbound authenticated connection. That bridge is not implemented in this integration yet.
