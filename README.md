...
Fleet Telemetry is optional. It streams state updates from the vehicle and avoids routine `vehicle_data` requests after the private telemetry bridge is enabled.

1. Open the Tesla Vehicle Command integration, select **Configure**, and set a public telemetry hostname and TCP port. Reload the integration to create its dedicated telemetry CA and server certificate.
2. In the Add-on Store, start **Tesla Fleet Telemetry Receiver** from step 3.
3. Create a public DNS record for the telemetry hostname and forward the configured TCP port directly to the receiver. Tesla must reach it without Cloudflare Access, HTTP routing, or a browser login flow.
4. Call `tesla_vehicle_command.configure_fleet_telemetry` in **Developer tools > Actions**, supplying the vehicle VIN.

The telemetry receiver's decoded-record endpoint is internal only. Do not publish its ZMQ port or reuse the command-proxy certificate for telemetry.
